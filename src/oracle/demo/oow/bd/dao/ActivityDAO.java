	...
	public void insertCustomerActivity(ActivityTO activityTO) {
		...
		if (activityTO != null) {
			...
			if (custId > 0 && movieId > 0) {
				...
				switch (activityType) {
				case STARTED_MOVIE:
					// insert movie entry into customer's currently watching
					// list
					// key = KeyUtil.getCustomerCurrentWatchListKey(custId,
					// movieId,activityTable);
					...
					break;
				case PAUSED_MOVIE:
					// update the current position of the movie into current
					// watch list
					activityTO.setTableId(KeyConstant.CUSTOMER_CURRENT_WATCH_LIST);
					value = activityTable.createRowFromJson(activityTO.toJsonString(), false);
					super.getKVStore().getTableAPI().put(value, null, null);
